...
import { Send, Paperclip, X, Code, Mic, FileText, Table, BarChart3, ImageIcon, PenLine } from 'lucide-react';
import { Message, TextContent, Content, ChartContent, DocumentContent, ImageContent, AudioContent, DiagramContent, SpreadsheetContent, CodeContent } from '@/types/core';
import { ContentSuggestion } from '@/components/context/ContextAnalyzer';
import Image from 'next/image';
import { ScrollArea } from '../ui/scroll-area';

// Types for state
type ContentType = 'text' | 'code' | 'image' | 'audio' | 'document' | 'spreadsheet' | 'chart' | 'diagram';
...
export default function AdaptiveMultiModalChat({
  ...
  const [annotations, setAnnotations] = useState<Record<string, MessageAnnotation[]>>({});
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const aiMessage: Message = {
        ...
        content: {
          type: 'text',
          data: contextualAnswer || "I don't have a specific answer for that. Is there something else you'd like to know?",
        },
      };
  ...
  const createContent = (type: ContentType, value: string, preview: PreviewData | null): Content => {
    switch (type) {
      case 'text':
        return { type, data: value } as TextContent;
      case 'code':
        return { type, language: preview?.language || 'javascript', data: value } as CodeContent;
      case 'image':
        return { 
          type, 
  ...
  return (
    <div className={`flex flex-col h-full ${containerClasses[currentViewMode]}`}>
      {/* Messages container */}
      <div 
        ref={containerRef}
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center p-4">
            ...
            <p className="text-gray-500 dark:text-gray-400 max-w-md">
              Send a message to start chatting. You can use different content types including text, code, images, audio, and more.
            </p>
          </div>
        ) : (
          <ScrollArea className="h-[calc(100vh-180px)] md:h-[calc(100vh-210px)] w-full">
            {messages.map(message => (
              <div 
                key={message.id} 
                className={`mb-4 ${message.sender === 'user' ? 'ml-8 md:ml-16' : 'mr-8 md:mr-16'}`}
                onClick={isAnnotating && selectedMessage !== message.id ? () => handleAnnotate(message.id) : undefined}
              >
                <div 
                  ...
                  } ${message.id === selectedMessage ? 'ring-2 ring-blue-500' : ''} ${isAnnotating ? 'cursor-pointer' : ''}`}
                >
                  ...
                  {/* Annotations */}
                  {annotations[message.id] && annotations[message.id].length > 0 && (
                    <div className="mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                      <div className="flex items-center mb-1">
                        <PenLine size={14} className="mr-1 text-blue-500" />
                        <span className="text-xs font-medium text-gray-500">Annotations</span>
                      </div>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/30 p-3 border-t border-blue-100 dark:border-blue-800">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <PenLine size={16} className="mr-2 text-blue-600 dark:text-blue-400" />
      ...
      {contentSuggestion && !isAnnotating && (
        <div className="bg-gray-50 dark:bg-gray-800/50 p-2 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <button 
              onClick={() => setInputType(contentSuggestion.type as ContentType)}
            ...
            >
              {contentSuggestion.type === 'code' && <Code size={14} className="mr-1" />}
              {contentSuggestion.type === 'image' && <ImageIcon size={14} className="mr-1" />}
              {contentSuggestion.type === 'audio' && <Mic size={14} className="mr-1" />}
              {contentSuggestion.type === 'document' && <FileText size={14} className="mr-1" />}
              {contentSuggestion.type === 'spreadsheet' && <Table size={14} className="mr-1" />}
              {contentSuggestion.type === 'chart' && <BarChart3 size={14} className="mr-1" />}
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="relative">
          <div className="flex bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            ...
            <div className="flex-grow">
              ...
              {(inputType === 'audio' || inputType === 'document' || inputType === 'spreadsheet' || inputType === 'chart') && (
                <div className="p-3 w-full">
                  <button
                    type="button"
                    onClick={() => fileInputRef.current?.click()}
              ...
              <input
                ...
                accept={
                  ...
                  inputType === 'document' ? '.pdf,.doc,.docx,.txt' :
                  inputType === 'spreadsheet' ? '.csv,.xlsx' :
                  inputType === 'code' ? '.js,.ts,.jsx,.tsx,.py,.java' : '*'
                }
              />
            </div>
        ...
        <div className="bg-gray-100 dark:bg-gray-900 p-3 rounded">
          ...
          {content.transcription && (
            <p className="text-sm mt-2">
              <span className="font-medium">Transcription:</span> {content.transcription}
            </p>
          )}
        </div>
      );
      ...
      return (
        <div className="bg-gray-100 dark:bg-gray-900 p-3 rounded">
          <div className="flex items-center mb-2">
            ...
            <span className="font-medium">{content.fileName || 'Document'}</span>
          </div>
